...
void cwStrategyDemo::OnBar(cwMarketDataPtr pPriceData, int iTimeScale, cwBasicKindleStrategy::cwKindleSeriesPtr pKindleSeries) {
	...
	code2data[InstrumentID] = pPriceData;*/
...
void cwStrategyDemo::UpdateBarData() {
	...
	// 获取60天近期交易日序列
	std::string sqlTradingDay = std::format("select * from tradeday where tradingday < {} order by tradingday DESC Limit 60;", cursor_str);
	std::vector<std::string> tradeDate;
	sqlite3_stmt* stmt1 = nullptr;
	if (sqlite3_prepare_v2(cnnSys, sqlTradingDay.c_str(), -1, &stmt1, nullptr) == SQLITE_OK) {
		while (sqlite3_step(stmt1) == SQLITE_ROW) {
			tradeDate.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt1, 0)));
		}
	}
	sqlite3_finalize(stmt1);
	std::sort(tradeDate.begin(), tradeDate.end());

	// 日期字符串映射
	std::map<std::string, std::string> calendar;
	for (const auto& date : tradeDate) {
		std::string date_map = date.substr(0, 4) + "-" + date.substr(4, 2) + "-" + date.substr(6, 2);
		calendar[date_map] = date;
	}

	//加载当天策略配置参数
	std::cout << "LOADING - verDictCur >>>" << std::endl;
	std::string sqlFacTag = std::format("select contract, Fac, slc from facTag where tradingday={};", cursor_str); //调取当天的ver
	...
	if (sqlite3_prepare_v2(cnn, sqlFacTag.c_str(), -1, &stmt2, nullptr) == SQLITE_OK) {
		while (sqlite3_step(stmt2) == SQLITE_ROW) {
			...
			int Rs = std::stoi(ver.substr(0, ver.find('-')));
			int Rl = std::stoi(ver.substr(ver.find('-') + 1));
	...
	//g.futureinfo = 加载合约信息:交易所，交易乘数，价位，保证金比率
	...
	//记录前一天交易日
	std::string sqlMainContract = "select tradingday, prefix, code, factor,accfactor from TraderOvk where tradingday>='" + tradeDate.front() + "' and tradingday<='" + tradeDate[tradeDate.size() - 1] + "';";
	...
	for (const std::string& date : tradeDate) {
		...
		std::string codeSql = boost::algorithm::join(codes, "','");
		//读取his_data
		std::string dbPath = "his" + date.substr(0, 4) + ".db";
		sqlite3* cnnBar;
	...
	// g.countLimt = 根据前一天的收盘价计算当天交易每个合约对应的数量
	...
	sqlite3_close(cnn);
	std::cout << "### " << "OnReady" << " Success >>>" << std::endl;
}
