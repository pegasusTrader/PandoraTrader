	...
	if (mydb) {
		...
		if (sqlite3_prepare_v2(mydb, tar_contract_sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
			while (sqlite3_step(stmt) == SQLITE_ROW) {
				tar_contract_list.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0)));
			}
		}
		sqlite3_finalize(stmt);
		for (auto& contract : tar_contract_list) {
			std::string ret_sql = std::format("SELECT ret FROM {} ORDER BY tradingday,timestamp ",contract);
			sqlite3_stmt* stmt = nullptr;
			if (sqlite3_prepare_v2(mydb, ret_sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
				while (sqlite3_step(stmt) == SQLITE_ROW) {
					retBar[contract].push_back(sqlite3_column_double(stmt, 0));
				}
			}
			sqlite3_finalize(stmt);
		}
